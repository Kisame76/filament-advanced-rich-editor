...
| **Images** | `attachFiles` uploads a file — every time, even the same one | [A browser over the files already on the server](docs/documentation.md#media-browser) — pictures, video and audio behind one button, with a still on every tile, an alt text saved against the file rather than the insert, and YouTube or Vimeo embeds kept in the library beside everything else — [a caption beside the alt text](docs/documentation.md#images), and [Spatie Media Library](docs/documentation.md#spatie-media-library) as an option |
