...
| --- | --- | --- |
| **Toolbar** | A fixed default row. Groups and dropdowns exist, but each one is assembled by hand | [Five named presets](docs/documentation.md#toolbar-presets) covering all four bars at once, [tokens](docs/documentation.md#rearranging-the-toolbar) that expand into whole groups, [dividers](docs/documentation.md#dividers), [two overflow menus](docs/documentation.md#the-tools-menu), a [pinned corner](docs/documentation.md#pinned-buttons), [start, centre, end or spread across the width](docs/documentation.md#toolbar-alignment), [sticky while scrolling](docs/documentation.md#sticky-toolbar), and [one config file](docs/documentation.md#configuration) for every field in the project |
...
| **Block layout** | Four separate alignment buttons, three of them in the default row | [One alignment dropdown](docs/documentation.md#alignment) showing the alignment the caret is in, [line spacing](docs/documentation.md#line-spacing) beside it, and [indenting](docs/documentation.md#indenting) that steps a paragraph in and nests a list item — same two buttons, same two keys, one line to switch on |
...
| **Images** | `attachFiles` uploads a file — every time, even the same one | [A browser over the files already on the server](docs/documentation.md#media-browser) — pictures, video, audio and [documents](docs/documentation.md#documents) behind one button, with or without Spatie, a still on every tile, an alt text saved against the file rather than the insert, a pdf or a spreadsheet inserted as a download card, and YouTube or Vimeo embeds kept in the library beside everything else — [a caption beside the alt text](docs/documentation.md#images), and [Spatie Media Library](docs/documentation.md#spatie-media-library) as an option |
...
| **Laying a picture out** | Alt text, and the size you dragged it to | + [the side the text runs past](docs/documentation.md#where-a-picture-sits), [a caption](docs/documentation.md#captions), [the measured size](docs/documentation.md#size-and-lazy-loading) so the page stops jumping when it loads, [an address to point at](docs/documentation.md#a-picture-that-points-somewhere), and [a mark for the ones that carry nothing](docs/documentation.md#a-picture-that-carries-nothing) — which is what stops the accessibility check reporting a deliberate empty `alt` as a forgotten one |
| **Links** | A URL and "open in a new tab" | + [`rel`, `referrerpolicy` and `hreflang`](docs/documentation.md#links), with `noopener noreferrer` added automatically to anything opening in a new window, and [a search over your own records](docs/documentation.md#linking-to-a-record-instead-of-typing-a-url) instead of a typed URL |
| **Video** | — | [Paste a YouTube or Vimeo link](docs/documentation.md#video-embeds) and get a player, timestamp included, through the cookie-free host — or [point at a file on your own server](docs/documentation.md#video-and-audio-you-host), video or audio, with a poster and no third party involved |
...
| **Formatting what you selected** | `->floatingToolbars()` exists; the only bar Filament ships is the one over a table cell | [A bar over the selected text](docs/documentation.md#toolbar-over-a-selection) carrying the styles, the marks, the link and both colour pickers — in a heading and a list item as well as a paragraph, taking the same tokens the main toolbar does |
| **Repeating formatting** | — | [The brush from Word](docs/documentation.md#format-brush) — pick the look up at one passage and put it down on another, for one stroke or until you switch it off. Character formatting, read off *this* field's schema rather than a list, and it declines the three marks that cannot be carried honestly: a code run takes every other mark off with it, a link is an address rather than a look, and a language is what a passage *is* |
...
| **Seeing the page** | — | [A preview in an isolated frame](docs/documentation.md#preview) drawn with *your* stylesheet rather than the editor's — the panel's CSS cannot reach inside it, which is what makes it the page rather than a flattering copy of it |
| **How long it is** | `minLength()` / `maxLength()` count characters, and an untouched field still stores one empty paragraph | [A visible counter](docs/documentation.md#character-count) in characters or words, [word counts and rules about what the document holds](docs/documentation.md#rules-about-the-content) — `->maxWords(60)`, `->mustContain('image')` — [an empty document that fails `required`](docs/documentation.md#required-and-what-counts-as-empty) instead of passing it, and [`null` in the column](docs/documentation.md#storing-nothing-instead-of-pp) when you ask for it |
...
| **Editor chrome** | — | [Fullscreen](docs/documentation.md#fullscreen), [a maximum height](docs/documentation.md#maximum-height), [a shortcut list](docs/documentation.md#help), [emoji](docs/documentation.md#emoji), [special characters](docs/documentation.md#special-characters) and [today's date](docs/documentation.md#date-and-time) in the application's language and Filament's display timezone |
| **Rendering** | `toHtml()`, `toText()`, `toArray()` | + [heading anchors](docs/documentation.md#anchors), [a table of contents](docs/documentation.md#table-of-contents) from the same slug pass, [column widths that reach the page](docs/documentation.md#table-column-widths), [Markdown](docs/documentation.md#markdown) with the checkboxes intact — in both directions, and the way back in is the one that has to be careful, because a footnote read as a link reference definition deletes the note and leaves valid markup behind — and [an excerpt](docs/documentation.md#excerpts) for the meta description — plus the two repairs that made one possible, a sentence that stops breaking apart at a link and entities that are spelled out |
...
Markdown in and out, and the config file.
